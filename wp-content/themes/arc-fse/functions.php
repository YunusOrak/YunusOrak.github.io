...
function arc_fse_admin_scripts( $hook_suffix ) {
	...
	if ( 'appearance_page_arc-fse-info' === $hook_suffix ) {
		$deps = array( 'wp-api-fetch' );
		$handle = 'arc-fse-admin';
		wp_enqueue_script( $handle, get_template_directory_uri() . '/assets/js/admin' . $min . '.js', $deps, date( 'Ymd-Gis', filemtime( get_theme_file_path( 'assets/js/admin.js' ) ) ) );
	}
}
